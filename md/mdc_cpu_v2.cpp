...
class md_system_bs {
    public:
        ...
        mutable std::vector<index_hash> cell_idx;
        mutable std::vector<index_type> cell_coo;
        mutable std::vector<size_t>  cells_begin;
...
#pragma omp parallel for schedule(dynamic, 512)
            for(size_t i = 0; i < prm.n; ++i) {
                size_t i1 = static_cast<size_t>(x[i][0] / prm.cell_size);
                size_t i2 = static_cast<size_t>(x[i][1] / prm.cell_size);

                cell_coo[i]      = index_type(i1, i2);
                cell_idx[i].hash = get_cell_idx(cell_coo[i]);
                cell_idx[i].pos  = i;
            }

            // Sort particles based on hash
            std::sort(cell_idx.begin(), cell_idx.end(),
                    [](index_hash h1, index_hash h2) {
                        return h1.hash < h2.hash;
                    });

            // Reorder particle arrays into sorted order and find start and end of each cell
#pragma omp parallel for schedule(dynamic, 512)
            for(size_t i = 0; i < prm.n_cells; ++i) {

                struct hash_comp {
                    bool operator()(size_t i, index_hash h) const {
                        return i < h.hash;
                    }

                    bool operator()(index_hash h, size_t i) const {
                        return h.hash < i;
                    }
                };

                auto r = std::equal_range(cell_idx.begin(), cell_idx.end(), i, hash_comp());

                cells_begin[i] = std::distance(cell_idx.begin(), r.first);
                cells_end  [i] = std::distance(cell_idx.begin(), r.second);
            }
...
#pragma omp parallel for schedule(dynamic, 512)
            for(size_t idx = 0; idx < prm.n; ++idx) {
                point_type X     = x_bc[idx];
                point_type V     = v[idx];
                index_type index = cell_coo[idx];

                point_type A = prm.local_force( X , V );

                for( int i=-1 ; i<=1 ; ++i ) {
                    for( int j=-1 ; j<=1 ; ++j ) {
                        index_type cell_index = index + index_type(i, j);
                        size_t     cell_hash  = get_cell_idx(cell_index);

                        for(size_t ii = cells_begin[cell_hash], end = cells_end[cell_hash]; ii < end; ++ii) {
                            size_t jj = cell_idx[ii].pos;

                            if( jj == idx ) continue;

                            point_type Y = x_bc[jj];

                            if( cell_index[0] >= prm.n_cell_x ) Y[0] += prm.x_max;
                            if( cell_index[0] < 0 )             Y[0] -= prm.x_max;
                            if( cell_index[1] >= prm.n_cell_y ) Y[1] += prm.y_max;
                            if( cell_index[1] < 0 )             Y[1] -= prm.y_max;

                            A += prm.interaction(X, Y);
                        }
                    }
                }
...
#pragma omp parallel for schedule(dynamic, 512)
        ...
        inline size_t get_cell_idx(index_type index) const {
            size_t i1 = check_interval(index[0], prm.n_cell_x);
            size_t i2 = check_interval(index[1], prm.n_cell_y);

            return i1 * prm.n_cell_y + i2;
        }
};
...
int main( int argc , char *argv[] ) {
    ...
    log_perf("cpu_v2", n, m * 5, timer.elapsed());
    std::cout << std::endl;
}
